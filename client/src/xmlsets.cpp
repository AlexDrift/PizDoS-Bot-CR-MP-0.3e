...
{
	// load xml
	if(!xmlSettings.LoadFile("PizDoS_Bot.xml"))
	{
		MessageBox(NULL, "Failed to load the config file", "Error", MB_ICONERROR);
		ExitProcess(0);
	}

	TiXmlElement* rakSAMPElement = xmlSettings.FirstChildElement("PizDoS_Bot");
	if(rakSAMPElement)
	{
		// get console
		...
		// get followplayer
		strcpy(settings.szFollowingPlayerNameq, (char *)rakSAMPElement->Attribute("followplayerq"));
