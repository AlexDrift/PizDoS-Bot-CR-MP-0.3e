	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			case ID_DISCONNECTION_NOTIFICATION:
				Log("[PizDoS Bot] Подключение было закрыто сервером. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_CONNECTION_BANNED:
				Log("[PizDoS Bot] Я забанен. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;			
			case ID_CONNECTION_ATTEMPT_FAILED:
				Log("[PizDoS Bot] Попытка подключения не удалась. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				Log("[PizDoS Bot] Сервер переполнен игроками. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_INVALID_PASSWORD:
				Log("[PizDoS Bot] Неверный пароль. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_CONNECTION_LOST:
				Log("[PizDoS Bot] Связь была потеряна. Повторное подключение через %d секунд.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
