...
void InitGame(RPCParameters *rpcParams)
{
	...
	{
		sprintf(szTitle, "%s (%d) - %.16s - PizDoS Bot %s", g_szNickName, g_myPlayerID, g_szHostName, RAKSAMP_VERSION);
		SetConsoleTitle(szTitle);
		Log("Подключено к %.64s\n", g_szHostName);
	}
	else
	{
		sprintf(szTitle, "%s (%d) - PizDoS Bot %s", g_szNickName, g_myPlayerID, RAKSAMP_VERSION);
		SetWindowText(hwnd, szTitle);
		Log("Подключено к %.64s", g_szHostName);
	}
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		Log("[PizDoS Bot] Bad SA-MP version.");
	}
	else if(byteRejectReason==REJECT_REASON_BAD_NICKNAME)
	{
		...
		Log("[PizDoS Bot] Bad nickname. Changing name to %s", szNewNick);
	...
	{
		Log("[PizDoS Bot] Bad mod version.");
	}
	else if(byteRejectReason==REJECT_REASON_BAD_PLAYERID)
	{
		Log("[PizDoS Bot] Bad player ID.");
	}
	else
...
void Chat(RPCParameters *rpcParams)
{
	...
	Log("[ЧАТ] %s: %s", playerInfo[playerId].szPlayerName, szText);
